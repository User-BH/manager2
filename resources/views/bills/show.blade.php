...
<div class="mx-auto max-w-3xl space-y-4">
    ...
    <x-card>
        <div class="mb-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
            <div><p class="text-slate-400">واحد</p><p class="font-medium">{{ $bill->unit->label() }}</p></div>
            <div><p class="text-slate-400">مهلت پرداخت</p><p class="font-medium">{{ Jalali::date($bill->due_date) }}</p></div>
            <div><p class="text-slate-400">مبلغ کل</p><p class="font-medium tabular-nums">{{ Jalali::money($bill->total_amount) }} {{ $currency }}</p></div>
            <div><p class="text-slate-400">مانده</p><p class="font-medium tabular-nums text-rose-600 dark:text-rose-400">{{ Jalali::money($bill->remaining()) }} {{ $currency }}</p></div>
        </div>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                @foreach ($bill->breakdown ?? [] as $item)
                    <tr>
                        ...
                        <td class="py-2 text-left tabular-nums {{ $item['amount'] < 0 ? 'text-emerald-600' : '' }}">{{ Jalali::money($item['amount']) }}</td>
                    </tr>
                @endforeach
            </tbody>
            <tfoot>
        ...
        <div class="mt-3 flex flex-wrap gap-4 text-xs text-slate-400">
            ...
            @if ($bill->penalty_amount > 0)<span class="text-rose-500">جریمه: {{ Jalali::money($bill->penalty_amount) }}</span>@endif
            @if ($bill->discount_amount > 0)<span class="text-emerald-500">تخفیف: {{ Jalali::money($bill->discount_amount) }}</span>@endif
        </div>
    </x-card>
