...
<div class="space-y-4">
    ...
    <x-card title="در انتظار تایید" subtitle="رسیدهای آپلودشده توسط ساکنین">
        @if ($pending->isEmpty())
            <p class="py-8 text-center text-sm text-faint">پرداخت در انتظار تاییدی وجود ندارد.</p>
        @else
            <div class="space-y-3">
                    ...
                    <div class="rounded-xl border border-line p-4">
                        <div class="flex flex-wrap items-start justify-between gap-3">
                            <div>
                                ...
                                @if ($p->description)<p class="mt-1 text-xs text-faint">توضیح: {{ $p->description }}</p>@endif
                                @if ($p->receipt_paid_on)<p class="mt-1 text-xs text-faint">تاریخ واریز: {{ Jalali::date($p->receipt_paid_on) }}</p>@endif
                            </div>
    ...
    <x-card title="پرداخت‌های اخیر">
        @if ($recent->isEmpty())
            <p class="py-6 text-center text-sm text-faint">موردی نیست.</p>
        @else
