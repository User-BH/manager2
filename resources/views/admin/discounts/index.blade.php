...
<div class="space-y-4">
    ...
    <p class="text-sm text-faint">تخفیف برای هر واحد در دوره‌ی مشخص ثبت می‌شود و هنگام صدور قبض از مبلغ کل کسر می‌گردد.</p>
        ...
        <div class="lg:col-span-2">
            <x-card title="تخفیف‌های ثبت‌شده‌ی این دوره">
                @if ($discounts->isEmpty())
                    <p class="py-8 text-center text-sm text-faint">تخفیفی ثبت نشده است.</p>
                @else
                        ...
                        <tbody class="divide-y divide-line">
                            @foreach ($discounts as $d)
                                <tr>
                                    ...
                                    <td class="py-2.5 tabular-nums text-success">{{ Jalali::money($d->amount) }}</td>
                                    <td class="py-2.5 text-muted">{{ $d->reason ?: '-' }}</td>
